...
const std::vector<std::string> FONT_PATHS = {
    ...
    "/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf",  // Another Linux path
...
public:
    ...
    int predictedYPosition() const {
        if (xVel == 0) return SCREEN_HEIGHT / 2;
    ...
    void moveAIPaddle() {
        ...
        switch (aiDifficulty) {
            case Difficulty::EASY:
                ...
                if (rand() % 10 < 3) {
                    rightPaddle.yVel = (targetY > paddleCenter) ? PADDLE_SPEED / 2 : -PADDLE_SPEED / 2;
                } else {
                    rightPaddle.yVel = 0;
                }
                break;
            ...
            case Difficulty::MEDIUM:
                ...
                if (abs(targetY - paddleCenter) > PADDLE_HEIGHT / 4) {
                    rightPaddle.yVel = (targetY > paddleCenter) ? 
...
public:
    Pong(bool playerTwoAI = true, Difficulty difficulty = Difficulty::MEDIUM) : 
        ...
        font(nullptr),
        running(true),
        isPlayerTwoAI(playerTwoAI),
        aiDifficulty(difficulty),
        leftPaddle(50, SCREEN_HEIGHT / 2 - PADDLE_HEIGHT / 2),
        rightPaddle(SCREEN_WIDTH - 50 - PADDLE_WIDTH, SCREEN_HEIGHT / 2 - PADDLE_HEIGHT / 2),
        leftScore(0),
        rightScore(0)
    {
        // Initialize SDL
...
void handleEvents() {
    ...
    void renderScore() {
        ...
        SDL_Surface* leftSurface = TTF_RenderText_Solid(font, leftScoreText.c_str(), white);
        
        if (!leftSurface) {
            std::cerr << "Unable to render left score text! SDL_ttf Error: " << TTF_GetError() << std::endl;
            return;
        }
        ...
        leftScoreRect.y = 20;
        SDL_QueryTexture(leftTexture, NULL, NULL, &leftScoreRect.w, &leftScoreRect.h);
        SDL_RenderCopy(renderer, leftTexture, NULL, &leftScoreRect);

        // Render right score
        std::string rightScoreText = std::to_string(rightScore);
        SDL_Surface* rightSurface = TTF_RenderText_Solid(font, rightScoreText.c_str(), white);
        
        if (!rightSurface) {
            std::cerr << "Unable to render right score text! SDL_ttf Error: " << TTF_GetError() << std::endl;
        ...
        rightScoreRect.y = 20;
        SDL_QueryTexture(rightTexture, NULL, NULL, &rightScoreRect.w, &rightScoreRect.h);
    ...
    void renderDifficultyInstructions() {
        ...
        SDL_Surface* difficultySurface = TTF_RenderText_Solid(font, difficultyText.c_str(), white);
        
        if (!difficultySurface) {
            std::cerr << "Unable to render difficulty text! SDL_ttf Error: " << TTF_GetError() << std::endl;
            return;
        }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--2player" || arg == "-2") {
            ...
            std::cout << "  Right Paddle: " << (isPlayerTwoAI ? "AI (Adjust difficulty with 1-3 keys)" : "Up/Down arrow keys") << std::endl;
            return 0;
        }
    }
